...
One could attempt making the same statement about WebAssembly, it is by design
executed in an environment separate from the webpage; though this is not as
much of an issue.
...
The defined goals of the WebAssembly project details the process for evolving
...
In addition to verifying proofs, many proof assistants also provide
the use of proof tactics.
% TODO
...
% TODO: Talk about what Soundness actually means / what it gets you
...
mechanization included two separate modules which were also proven sound: an
executable type checker\cite[p. 61]{WattMechanizing} and an executable
...
tactics which can perform the legwork of applying individual steps of logic in
order to arrive at a given assertion.
...
Because the non-saturating truncation instructions can fail, resulting in a
trap, \texttt{cvt} returns an \texttt{Option} value, indicating \texttt{Some}
value if the conversion succeeded or \texttt{None} otherwise, indicating an
impossible conversion, or a conversion operation that failed and trapped.
...
\texttt{return\_invoke} defines the execution semantics of the additional
behavior.
...
% Another paragraph summarizing what I learned
% - What's the point?
