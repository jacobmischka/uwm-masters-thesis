...
making mistakes regarding to unexpected or unconsidered possibilities
...
perform the addition; \texttt{add} may in fact subtract the numbers instead,
still returning a number as declared but not the value expected by the caller.
...
mechanization included two separate modules which were also proven sound with respect to
the mechanized specification: an executable type checker\cite[p.
...
evaluation, a new result step type \texttt{RSTailInvoke} had to be introduced.
The existing returning result step type \texttt{RSReturn} was not sufficient as
it only contains the list of returned values to the next step of evaluation,
